...
const exportData = async (format) => {
  try {
    ...
    if (result.success) {
      ElMessage.success(`Export successful, the file is ready for download`)
      // 实际项目中，应该触发文件下载
...
const initTrendChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: trendChart')
    return
  }
...
const initDistributionChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: distributionChart')
    return
  }
...
const initFailureReasonsChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: failureReasonsChart')
    return
  }
  ...
  const option = {
    title: {
      text: 'Distribution of Failure Reasons',
      left: 'center'
    },
    tooltip: {
      trigger: 'item',
...
const initResponseTimeChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: responseTimeChart')
    return
  }
...
const initPeakTimeChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: peakTimeChart')
    return
  }
  ...
  const option = {
    title: {
      text: 'Peak Time for Sending Messages',
      left: 'center'
    },
    tooltip: {
      trigger: 'axis',
...
const initActivityChart = () => {
  ...
  if (!chartDom) {
    console.error('Cannot find the chart DOM element: activityChart')
    return
  }
  ...
  const option = {
    ...
    legend: {
      data: ['Total Received', 'Read Rate(%)', 'Average Reading Time(s)'],
      top: 'bottom'
    },
    grid: {
      left: '3%',
  ...
  <div class="statistics-container">
    <el-row :gutter="20">
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <h2 class="page-title">Statistical Analysis</h2>
      </el-col>
    </el-row>
        ...
        <el-card shadow="never">
          <div class="filter-container">
            ...
            <div class="filter-item date-range">
              ...
              />
              <el-button type="primary" @click="filterByDateRange" :icon="Search">Search</el-button>
            </div>
            
            <div class="filter-item dimension-filter">
              <el-select v-model="selectedSystem" placeholder="System" @change="changeDistributionDimension" clearable>
              ...
              <el-select v-model="selectedType" placeholder="Type" @change="changeDistributionDimension" clearable>
                <el-option label="All" value="all" />
                <el-option
              ...
              <el-select v-model="selectedChannel" placeholder="Channel" @change="changeDistributionDimension" clearable>
                <el-option label="All" value="all" />
                <el-option
            ...
            <div class="filter-item export-buttons">
              <el-dropdown @command="exportData">
                <el-button type="success" :icon="Download">
                  Export Data<el-icon class="el-icon--right"><arrow-down /></el-icon>
                </el-button>
                <template #dropdown>
                  <el-dropdown-menu>
