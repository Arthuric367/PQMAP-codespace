...
const recipients = ref([
  {
    ...
    subscriptionSettings: [
      { typeId: 1, channels: ['email', 'sms'] },
      { typeId: 3, channels: ['email'] }
    ]
  },
  {
    id: 2,
...
const messageTypes = ref([
  { id: 1, name: 'System Notification', icon: 'Bell', color: '#409EFF' },
  { id: 2, name: 'Alert Message', icon: 'Warning', color: '#F56C6C' },
  { id: 3, name: 'Marketing Message', icon: 'Promotion', color: '#67C23A' },
  { id: 4, name: 'Task Reminder', icon: 'Calendar', color: '#E6A23C' },
  { id: 5, name: 'Account Security', icon: 'Lock', color: '#909399' }
])

// 通知渠道列表
const notificationChannels = ref([
  { id: 'email', name: 'Email', icon: 'Message' },
  { id: 'sms', name: 'SMS', icon: 'ChatDotRound' },
  { id: 'system_notification', name: 'In-system', icon: 'Bell' },
])
...
const saveRecipient = () => {
  ...
  if (!currentRecipient.value.name) {
    ElMessage.warning('Please enter the recipient name')
    return
  }
  ...
  if (dialogMode.value === 'add') {
    ...
    if (index !== -1) {
      ...
      ElMessage.success('Recipient updated')
    }
  }
...
const toggleRecipientStatus = (recipient) => {
  ...
  if (index !== -1) {
    ...
    ElMessage.success(`Recipient is ${newStatus === 'enabled' ? 'Active' : 'Disabled'}`)
  }
}
...
const syncFromDirectory = () => {
  ElMessage.info('Synchronizing recipient information from the corporate address book...')
  loading.value = true
  
  // 模拟同步过程
  setTimeout(() => {
    // 实际项目中应调用API同步数据
    ElMessage.success('Sync completed, 3 recipients added')
    loading.value = false
  }, 2000)
}
  ...
  <div class="recipients-container">
    ...
    <el-card shadow="hover" class="filter-card">
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="12" :md="6" :lg="4" :xl="4">
          <el-select v-model="typeFilter" placeholder="Recipient Type" class="filter-select">
        ...
        <el-col :xs="24" :sm="12" :md="6" :lg="4" :xl="4">
          <el-select v-model="departmentFilter" placeholder="Department" class="filter-select">
            <el-option label="All" value="all" />
            <el-option
        ...
        <el-col :xs="24" :sm="12" :md="6" :lg="10" :xl="10" class="action-buttons">
          <el-button type="success" @click="syncFromDirectory" :loading="loading">
            <el-icon><RefreshRight /></el-icon>Sync Address Book 
          </el-button>
          <el-button type="primary" @click="openAddDialog">
            <el-icon><Plus /></el-icon>Add Recipients
          </el-button>
        </el-col>
      </el-row>
    </el-card>
    ...
    <el-card shadow="hover" class="recipient-list-card">
      ...
      >
        ...
        <el-table-column label="Type" width="100">
          <template #default="{ row }">
            <el-tag :type="row.type === 'personal' ? 'primary' : 'success'" effect="plain">
              {{ row.type === 'personal' ? 'Personal' : 'Group' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Subscribe/Mute" width="180">
          <template #default="{ row }">
            <div class="subscription-preview">
              <div v-if="row.subscriptions && row.subscriptions.length > 0">
                <span class="subscription-label">Subscribe:</span>
                <el-tag 
              ...
              <div v-if="row.blockedTypes && row.blockedTypes.length > 0">
                <span class="subscription-label">Mute:</span>
                <el-tag 
    ...
    <el-dialog
      ...
      :title="dialogMode === 'add' ? 'Add Recipient' : 'Edit Recipient'"
      width="70%"
      destroy-on-close
    >
      <el-tabs v-model="activeTab" type="border-card">
        <el-tab-pane label="Basic Info" name="basic">
          <el-form :model="currentRecipient" label-width="100px">
            <el-row :gutter="20">
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                <el-form-item label="Name" required>
                  <el-input v-model="currentRecipient.name" placeholder="Please input recipient name" />
                </el-form-item>
              </el-col>
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            ...
            <el-row :gutter="20">
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                <el-form-item label="Status">
                  <el-select v-model="currentRecipient.status" placeholder="Select status" style="width: 100%">
              ...
              <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" v-if="currentRecipient.type === 'personal'">
                <el-form-item label="Department">
                  <el-select v-model="currentRecipient.department" placeholder="Department" style="width: 100%">
                    <el-option
                      v-for="dept in departments"
                      :key="dept.id"
                      :label="dept.name"
              ...
              <el-row :gutter="20">
                <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                  <el-form-item label="Email">
                    <el-input v-model="currentRecipient.email" placeholder="Please input email" />
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                  <el-form-item label="Phone No.">
                    <el-input v-model="currentRecipient.phone" placeholder="Please input phone number" />
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              <el-row :gutter="20">
                <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                  <el-form-item label="User ID">
                    <el-input v-model="currentRecipient.userId" placeholder="Please input user ID（In-system）" />
                  </el-form-item>
                </el-col>
              </el-row>
            </template>
            ...
            <template v-else>
              <el-divider>Group Member</el-divider>
              
              <el-form-item label="Select Member" required>
                <el-select
        ...
        <el-tab-pane label="Subscription Settings" name="subscription">
          <div class="subscription-settings">
            <p class="subscription-tip">Here you can configure the message types and delivery channels that recipients subscribe to</p>
            
            <el-divider content-position="left">Message Subscription</el-divider>
                ...
                <div class="message-types-container">
                  ...
                  >
                    ...
                    <div class="type-actions">
                      ...
                      >
                        <el-radio label="subscribe">Subscribe</el-radio>
                        <el-radio label="neutral">No config</el-radio>
                        <el-radio label="block">Mute</el-radio>
                      </el-radio-group>
                    </div>
                    <div class="type-channels" v-if="isTypeSubscribed(type.id)">
                      <div class="channels-title">Channel：</div>
                      <el-checkbox-group 
            ...
            <el-divider content-position="left">Subscription Summary</el-divider>
            
            <el-row :gutter="20">
              <el-col :span="12">
                <h4>Subscribed Message Types</h4>
                <el-empty description="No Subscription Types" v-if="currentRecipient.subscriptions.length === 0" />
                <el-table v-else :data="currentRecipient.subscriptions.map(id => ({
                  ...
                  <el-table-column prop="name" label="Message Types">
                    <template #default="{ row }">
                  ...
                  <el-table-column label="Channel">
                    <template #default="{ row }">
                      ...
                      <span v-if="row.channels.length === 0">None</span>
                    </template>
                  </el-table-column>
                </el-table>
              </el-col>
              <el-col :span="12">
                <h4>Muted Message Types</h4>
                <el-empty description="No mute types" v-if="currentRecipient.blockedTypes.length === 0" />
                <el-table v-else :data="currentRecipient.blockedTypes.map(id => ({
                  ...
                  <el-table-column prop="name" label="Message Type">
                    <template #default="{ row }">
                      <div class="type-name-cell">
                        <el-icon><component :is="row.icon" /></el-icon>
                        <span>{{ row.name }}</span>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="recipientDialogVisible = false">Cancel</el-button>
          <el-button type="primary" @click="saveRecipient">Save</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
