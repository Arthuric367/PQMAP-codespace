...
const initChannelStatusChart = () => {
  ...
  if (!chartDom) {
    console.error('Chart DOM element not found: channelStatusChart')
    return
  }
    ...
    series: [
      {
        ...
        center: ['50%', '60%'],
        data: [
          { value: enabledCount, name: 'Active', itemStyle: { color: '#67C23A' } },
          { value: disabledCount, name: 'Disabled', itemStyle: { color: '#909399' } }
        ],
        emphasis: {
          itemStyle: {
            shadowBlur: 10,
...
const activities = ref([
  { 
    content: 'The system administrator created a new message template.', 
    timestamp: '2023-10-12 10:30:00',
    type: 'primary',
    icon: 'Files'
  },
  { 
    content: 'xxx system sent an alarm message.', 
    timestamp: '2023-10-12 09:15:00',
    type: 'danger',
    icon: 'Warning'
  },
  { 
    content: 'The system maintenance notification template has been updated.', 
...
const loadAnomalyAlerts = async () => {
  try {
    ...
    console.error('Failed to load abnormal warning data:', error)
  }
}
...
onMounted(() => {
  try {
    ...
    // 加载统计数据
    loadStats()
    console.log('统Data has been loaded:', stats)
    ...
    setTimeout(() => {
      try {
        console.log('Start initializing the chart...')
        console.log('Message:', messageStore.messages.length)
        console.log('Channel:', channelStore.channels.length)
        
        if (messageStore.messages.length > 0) {
          initMessageTypeChart()
        } else {
          console.warn('The message data is empty, so skip the message type chart initialization.')
        }
        
        if (channelStore.channels.length > 0) {
          initChannelStatusChart()
        } else {
          console.warn('Channel data is empty; skip channel status chart initialization.')
        }
    ...
    <el-row :gutter="20" class="stat-cards">
      ...
      <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6">
        <el-card shadow="hover" class="stat-card">
          ...
          <div class="stat-info">
            <div class="stat-title">Template Count</div>
      ...
      <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6">
        <el-card shadow="hover" class="stat-card">
          ...
          <div class="stat-info">
            <div class="stat-title">System Count</div>
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-card shadow="hover" class="alert-card">
          <div class="alert-header">
            ...
            <el-button type="primary" link @click="goToAnomalyAlerts">Details</el-button>
          </div>
          <div class="alert-content">
            <div class="alert-count">
              <span class="count-value">{{ activeAnomalyCount }}</span>
              <span class="count-text">Active Anomaly Count</span>
            </div>
